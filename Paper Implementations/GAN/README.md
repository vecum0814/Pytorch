...
다시 한번 정리하자면, 이미지 데이터의 분포를 근사하는 모델 G를 만드는 것이 생성 모델의 목표입니다. 모델 G가 잘 작동한다는 의미는 원래 이미지들의 분포를 잘 모델링할 수 있다는 것을 의미하는데, 아래에서 다룰 Generative Adversarial Networks가 대표적입니다. 
